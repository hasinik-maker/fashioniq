...
html, body, [class*="css"] { font-family: 'Inter', sans-serif; background: #fff; color: #000; }
[data-testid="stSidebar"] { display: none; }
header[data-testid="stHeader"] { display: none; }
[data-testid="stAppViewContainer"] { padding-top: 0 !important; }
.block-container { padding: 0 !important; max-width: 100% !important; }
.topnav { background: #000; width: 100%; padding: 0 32px; display: flex; align-items: center; justify-content: space-between; height: 56px; border-bottom: 2px solid #C41E3A; }
.topnav-logo { font-family: 'Playfair Display', serif; font-size: 1.4rem; font-weight: 900; color: #fff; }
.topnav-logo span { color: #C41E3A; }
.topnav-user { font-size: 0.72rem; color: #aaa; }
.topnav-user strong { color: #fff; }
.masthead { background: #000; color: #fff; padding: 40px 40px 32px; border-bottom: 3px solid #C41E3A; margin-bottom: 32px; }
...
.masthead-sub { font-size: 0.8rem; color: #aaa; margin-top: 8px; }
...
.login-wrap { max-width: 460px; margin: 60px auto; padding: 48px 40px; background: #fff; box-shadow: 0 4px 32px rgba(0,0,0,0.1); border-top: 4px solid #C41E3A; }
...
.stTextInput > div > div > input { border: 1px solid #ddd !important; border-radius: 2px !important; color: #000 !important; }
.stTextArea > div > div > textarea { border: 1px solid #ddd !important; border-radius: 2px !important; color: #000 !important; }
...
DEFAULT_USERS = {
    ...
    "seller1": {"password": "sell123", "role": "seller", "name": "Rajesh Gupta",  "email": "rajesh@email.com", "city": "Surat",     "business": "Rajesh Textiles"},
}

# ── User database (starts with defaults, grows with sign-ups) ─────────────────
...
if not st.session_state.logged_in:
    ...
    st.markdown("""
    <div style='background:#000;padding:18px 40px;border-bottom:2px solid #C41E3A;'>
    ...
    with col_m:
        ...
        if st.session_state.auth_mode == "login":
            ...
            role_sel = st.selectbox("I am a", ["Buyer — I want to purchase stock", "Seller — I want to sell wholesale"])
            ...
            if st.button("Sign In →", key="login_btn"):
                ...
                if matched_user:
                    uname, udata = matched_user
                    selected_role = "buyer" if "Buyer" in role_sel else "seller"
                    if udata["role"] == selected_role or udata["role"] == "admin":
                        ...
                        st.error("Role does not match this account. Please select the correct role.")
                else:
            ...
            <div style='text-align:center;margin-top:16px;font-size:0.72rem;color:#888;'>
                Don't have an account? Click <strong>Create Account</strong> above.
            </div>
            """, unsafe_allow_html=True)
        ...
        # ══════════════════════════════════════════════════════════════════
            ...
            <div class='login-wrap'>
                <div class='login-logo'>Trend<span>ora</span></div>
                <div class='login-sub'>Create your free account</div>
            </div>
            """, unsafe_allow_html=True)

            su_role     = st.selectbox("I am a", ["Buyer — I want to purchase stock", "Seller — I want to sell wholesale"], key="su_role")
            su_name     = st.text_input("Full Name", placeholder="e.g. Priya Sharma")
            su_business = st.text_input("Business Name", placeholder="e.g. Priya Boutique")
            su_city     = st.text_input("City", placeholder="e.g. Hyderabad")
            su_email    = st.text_input("Email Address", placeholder="e.g. priya@email.com")
            su_username = st.text_input("Choose a Username", placeholder="e.g. priya_boutique")
            su_password = st.text_input("Choose a Password", type="password")
            su_confirm  = st.text_input("Confirm Password", type="password")

            st.markdown("""
            <div style='background:#f9f9f9;padding:10px 14px;border-radius:2px;font-size:0.73rem;color:#555;margin:8px 0;'>
                🛡️ By creating an account you agree to Trendora's Terms of Service.
            ...
            if st.button("Create Account →", key="signup_btn"):
                ...
                if not su_email.strip() or "@" not in su_email:
                    errors.append("Please enter a valid email address.")
                if not su_username.strip():
                    errors.append("Username is required.")
                if len(su_username) < 3:
                    errors.append("Username must be at least 3 characters.")
                if su_username in st.session_state.user_db:
                    errors.append("This username is already taken. Please choose another.")
                if any(u["email"] == su_email for u in st.session_state.user_db.values()):
                    errors.append("An account with this email already exists.")
                if len(su_password) < 6:
                    errors.append("Password must be at least 6 characters.")
                if su_password != su_confirm:
                    errors.append("Passwords do not match.")
                if not su_business.strip():
                ...
                if flagged:
                    errors.append(f"Flagged content detected: {', '.join(flagged)}")
...
<div class='topnav'>
    ...
    <div class='topnav-user'>
        <strong>{user_info.get('name', st.session_state.username)}</strong>
        &nbsp;·&nbsp;{role.upper()}
        &nbsp;·&nbsp;{user_info.get('business', '')}
    </div>
</div>
""", unsafe_allow_html=True)
...
with nav_cols[-1]:
    if st.button("Sign Out", key="signout"):
        for key in ["logged_in", "username", "role", "page",
                    "chat_history", "messages", "pinterest_images", "auth_mode"]:
...
if page == "Trends":
    ...
    with c1: st.markdown("<div class='metric-card'><div class='metric-val'>8</div><div class='metric-lbl'>Active Trends</div><div class='metric-delta'>↑ 3 new this week</div></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='metric-card'><div class='metric-val'>4</div><div class='metric-lbl'>Urgent Alerts</div><div class='metric-delta'>Reorder now</div></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='metric-card'><div class='metric-val'>94</div><div class='metric-lbl'>Top Heat Score</div><div class='metric-delta'>Cargo Pants 🔥</div></div>", unsafe_allow_html=True)
    with c4: st.markdown("<div class='metric-card'><div class='metric-val'>₹2.1L</div><div class='metric-lbl'>Revenue at Risk</div><div class='metric-delta'>Dead stock items</div></div>", unsafe_allow_html=True)
    ...
    with cf1: cat_f = st.selectbox("Category", CATEGORIES, key="t_cat")
    with cf2: urg_f = st.selectbox("Urgency", ["All","HIGH","MEDIUM","LOW","DEAD_STOCK"], key="t_urg")
    ...
    bcolors= ["#C41E3A" if h>80 else "#B8964A" if h>60 else "#555" for h in heats]
    fig = go.Figure(go.Bar(x=heats, y=names, orientation='h', marker_color=bcolors,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>12-Week Outlook · Revenue · Audience Intelligence</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)
    ...
    c1,c2,c3,c4 = st.columns(4)
    with c1: st.markdown("<div class='metric-card'><div class='metric-val'>+41%</div><div class='metric-lbl'>Peak Demand Surge</div><div class='metric-delta'>Cargo Pants · W5</div></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='metric-card'><div class='metric-val'>₹2.45L</div><div class='metric-lbl'>June Revenue</div><div class='metric-delta'>↑ 24% vs target</div></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='metric-card'><div class='metric-val'>198</div><div class='metric-lbl'>Units Sold June</div><div class='metric-delta'>Best month ever</div></div>", unsafe_allow_html=True)
    with c4: st.markdown("<div class='metric-card'><div class='metric-val'>W6</div><div class='metric-lbl'>Peak Week</div><div class='metric-delta'>Highest demand point</div></div>", unsafe_allow_html=True)
    ...
    fig_rev = go.Figure()
    fig_rev.add_trace(go.Bar(x=rev["months"], y=rev["revenue"], name="Actual",
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("<div class='eyebrow'>◆ Audience</div><div class='section-title'>Who Is Buying</div>", unsafe_allow_html=True)
    col_a, col_b = st.columns(2)
    with col_a:
        ...
        fig_age.update_layout(title=dict(text="Age Groups", font=dict(size=14,color='#000')),
            paper_bgcolor='white', font=dict(color='#000'), margin=dict(l=10,r=10,t=40,b=10), height=260)
    ...
    st.markdown("<div class='alert alert-black'>👥 68% of buyers are women aged 18-34 in Hyderabad. Best categories for this audience: Crochet Skirts, Maxi Dresses, Co-ord Sets.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Live Inventory · Auto Alerts · Reorder Suggestions</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)
    ...
    if reorder: st.markdown(f"<div class='alert alert-red'>🚨 <strong>REORDER NOW:</strong> {', '.join([i['name'].split(' - ')[0] for i in reorder])}</div>", unsafe_allow_html=True)
    if dead:    st.markdown(f"<div class='alert alert-gold'>⚠️ <strong>DEAD STOCK:</strong> {', '.join([i['name'].split(' - ')[0] for i in dead])} — start clearance.</div>", unsafe_allow_html=True)
    if over:    st.markdown(f"<div class='alert alert-gold'>📦 <strong>OVERSTOCK:</strong> {', '.join([i['name'].split(' - ')[0] for i in over])} — pause reordering.</div>", unsafe_allow_html=True)

    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    total_val = sum(i["qty"]*i["cost"] for i in INVENTORY)
    c1,c2,c3,c4 = st.columns(4)
    with c1: st.markdown(f"<div class='metric-card'><div class='metric-val'>₹{total_val//1000}K</div><div class='metric-lbl'>Total Stock Value</div><div class='metric-delta'>At cost</div></div>", unsafe_allow_html=True)
    with c2: st.markdown(f"<div class='metric-card'><div class='metric-val'>{len(reorder)}</div><div class='metric-lbl'>Reorder Alerts</div><div class='metric-delta'>Urgent</div></div>", unsafe_allow_html=True)
    with c3:
        dv = sum(i['qty']*i['cost'] for i in dead)
        st.markdown(f"<div class='metric-card'><div class='metric-val'>₹{dv//1000}K</div><div class='metric-lbl'>Dead Stock Value</div><div class='metric-delta'>Run clearance</div></div>", unsafe_allow_html=True)
    with c4:
        fast = sum(1 for i in INVENTORY if i["velocity"]=="Fast")
        st.markdown(f"<div class='metric-card'><div class='metric-val'>{fast}</div><div class='metric-lbl'>Fast-Moving SKUs</div><div class='metric-delta'>High demand</div></div>", unsafe_allow_html=True)
    ...
    for item in inv_f:
        ...
        with st.expander(f"{icon}  {item['name']}  ·  {s}  ·  Trend Score {item['trend_alignment']}/100"):
            ...
            st.markdown(f"""
            <div style='margin:12px 0 4px;font-size:0.68rem;color:#888;text-transform:uppercase;'>Stock Level</div>
            ...
            if s=="REORDER NOW":
                ...
                st.markdown(f"<div class='alert alert-red' style='margin-top:8px;'>💡 Order <strong>{sq} units</strong> — est. cost ₹{sq*item['cost']:,}</div>", unsafe_allow_html=True)
            elif s=="DEAD STOCK":
                st.markdown(f"<div class='alert alert-gold' style='margin-top:8px;'>💡 Flash sale at ₹{int(item['selling_price']*0.7)} (30% off)</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Market Benchmarking · Competitor Ranges · Margin Optimisation</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)

    c1,c2,c3 = st.columns(3)
    with c1: st.markdown("<div class='metric-card'><div class='metric-val'>₹950</div><div class='metric-lbl'>Avg Market Price</div><div class='metric-delta'>All SKUs</div></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='metric-card'><div class='metric-val'>5</div><div class='metric-lbl'>Price Up Opportunities</div><div class='metric-delta'>+₹18K/month margin</div></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='metric-card'><div class='metric-val'>64%</div><div class='metric-lbl'>Avg Gross Margin</div><div class='metric-delta'>Trending products</div></div>", unsafe_allow_html=True)

    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("<div class='eyebrow'>◆ Per Product</div><div class='section-title'>Price Positioning</div>", unsafe_allow_html=True)
    ...
    for p in PRICING:
        ...
        with st.expander(f"{p['product']}  ·  Your Price ₹{p['your_price']}  ·  {opp}"):
            ...
            st.markdown(f"""
            <div style='margin:12px 0 4px;font-size:0.68rem;color:#888;text-transform:uppercase;'>Market Range  ·  Low ₹{p['low']} → High ₹{p['high']}</div>
            <div style='position:relative;background:#eee;border-radius:3px;height:10px;'>
    ...
    fig5.add_hline(y=50, line_dash="dash", line_color="#B8964A",
        annotation_text="Min target 50%", annotation_font_color="#000")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with cs: search = st.text_input("Search", placeholder="e.g. cargo pants, Surat…")
    with cc: city_f = st.selectbox("City", ["All","Surat","Jaipur","Mumbai","Tirupur"])
    with cm: cat_f  = st.selectbox("Category", CATEGORIES, key="wh_cat")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Connect buyers and sellers · All messages safety-monitored</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)
    ...
    contact_options = list(contacts) + [u for u in other_users if u not in contacts]

    selected_contact = st.selectbox("Conversation with", contact_options)
    ...
    for m in convo:
        sender_name = st.session_state.user_db.get(m["from"], {}).get("name", m["from"])
        if m["from"]==uname:
            ...
            st.markdown(f"<div><div class='chat-sender'>{sender_name} · {m['time']}</div><div class='chat-msg-bot'>{m['text']}</div></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.button("Send →", key="send_msg"):
        if new_msg.strip():
            ...
            if flagged:
                st.markdown(f"<div class='safety-banner'>🚫 Message blocked — flagged content: {', '.join(flagged)}</div>", unsafe_allow_html=True)
            else:
    ...
    st.markdown("<div class='alert alert-black' style='margin-top:16px;'>🛡️ All messages are monitored. Illegal content or fraud results in immediate account suspension.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Ask anything · Trend-aware · Inventory-smart · Always on</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)

    st.markdown(f"""
    <div style='background:#000;color:#fff;padding:16px 20px;border-radius:2px;margin-bottom:20px;border-left:4px solid #C41E3A;'>
        <div style='font-family:"Playfair Display",serif;font-size:1rem;font-weight:700;margin-bottom:4px;'>Hello, {user_info.get('name','there')} 👋</div>
        <div style='font-size:0.82rem;color:#aaa;'>I know your inventory, trends, prices, and suppliers. Ask me anything about your business.</div>
    </div>""", unsafe_allow_html=True)
    ...
    quick_qs = [
        ...
        "Best wholesaler for cargo pants?",
        "Who is my target audience?",
        "What is trending right now?",
        "How is my revenue performing?",
        "What to stock for festive season?",
    ]
    ...
    digests = [
        ("#C41E3A","🔴 URGENT","Stock out in 4 days","Baggy Cargo Pants, Crochet Mini Skirts, and Maxi Dresses running out. Contact Rajesh Textiles and Femina Fashion House today."),
        ("#2E7D32","💰 OPPORTUNITY","₹18K margin available","Raise prices on 5 products to market average — adds ₹18,000/month in pure margin."),
        ("#B8964A","📦 ACTION","₹70K dead stock","78 blazers unsold 90+ days. 30% clearance sale recovers ₹81K and frees capital."),
        ("#000","📈 TREND","Crochet peaks in 3 weeks","Highest-margin item at 71%. Only 6 units. Order 75 units from Femina immediately."),
    ]
    dc = st.columns(2)
    ...
    for news in fn:
        ...
        <div class='news-card'>
            <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;'>
                ...
                <span style='background:{ic};color:#fff;padding:2px 10px;border-radius:20px;font-size:0.6rem;font-weight:700;white-space:nowrap;margin-left:12px;'>{news['impact']}</span>
            </div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Manage listings · Track enquiries · Add new products</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)
    ...
    if my_wh:
        ...
        c1,c2,c3 = st.columns(3)
        with c1: st.markdown(f"<div class='metric-card'><div class='metric-val'>{len(wh['products'])}</div><div class='metric-lbl'>Active Listings</div><div class='metric-delta'>Live on marketplace</div></div>", unsafe_allow_html=True)
        with c2: st.markdown("<div class='metric-card'><div class='metric-val'>3</div><div class='metric-lbl'>New Enquiries</div><div class='metric-delta'>Respond within 24h</div></div>", unsafe_allow_html=True)
        with c3:
            tv = sum(p["moq"]*p["price_per_unit"] for p in wh["products"])
            st.markdown(f"<div class='metric-card'><div class='metric-val'>₹{tv//1000}K</div><div class='metric-lbl'>Min Order Value</div><div class='metric-delta'>All products</div></div>", unsafe_allow_html=True)

        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<div class='eyebrow'>◆ Current Listings</div><div class='section-title'>Your Products</div>", unsafe_allow_html=True)
        for prod in wh["products"]:
            with st.expander(f"  {prod['name']}  ·  ₹{prod['price_per_unit']}/unit  ·  MOQ {prod['moq']}"):
                ...
                st.markdown(f"<div style='background:#f9f9f9;padding:10px;font-size:0.82rem;color:#000;border-radius:2px;margin-top:8px;'>Fabric: {prod['fabric']} · Sizes: {prod['sizes']}</div>", unsafe_allow_html=True)
    else:
        c1,c2,c3 = st.columns(3)
        with c1: st.markdown("<div class='metric-card'><div class='metric-val'>0</div><div class='metric-lbl'>Active Listings</div><div class='metric-delta'>Add your first product</div></div>", unsafe_allow_html=True)
        with c2: st.markdown("<div class='metric-card'><div class='metric-val'>0</div><div class='metric-lbl'>Enquiries</div><div class='metric-delta'>List products to receive</div></div>", unsafe_allow_html=True)
        with c3: st.markdown("<div class='metric-card'><div class='metric-val'>New</div><div class='metric-lbl'>Account Status</div><div class='metric-delta'>Get verified below</div></div>", unsafe_allow_html=True)

    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("<div class='eyebrow'>◆ Add New</div><div class='section-title'>List a Product</div>", unsafe_allow_html=True)
    with st.form("new_product"):
        ...
        with c2:
            np_fabric = st.text_input("Fabric", placeholder="e.g. Cotton-Poly Blend")
            np_sizes  = st.text_input("Sizes Available", placeholder="e.g. S-XXL")
            np_colors = st.number_input("Number of Colors", min_value=1, value=5)
        np_desc = st.text_area("Description", placeholder="Describe your product…")
        ...
        if submitted and np_name:
            ...
            if flagged:
                ...
                st.success(f"✅ '{np_name}' listed on Trendora Marketplace! Buyers can now find and order your product.")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='masthead'>
        ...
        <div class='masthead-sub'>Incoming orders · Confirm · Dispatch · Track</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("<div class='page-wrap'>", unsafe_allow_html=True)

    c1,c2,c3 = st.columns(3)
    with c1: st.markdown("<div class='metric-card'><div class='metric-val'>3</div><div class='metric-lbl'>New Orders</div><div class='metric-delta'>Action required</div></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='metric-card'><div class='metric-val'>₹1.2L</div><div class='metric-lbl'>This Month</div><div class='metric-delta'>↑ 18% vs last month</div></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='metric-card'><div class='metric-val'>7</div><div class='metric-lbl'>Pending Dispatch</div><div class='metric-delta'>Ship within 48h</div></div>", unsafe_allow_html=True)

    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("<div class='eyebrow'>◆ Incoming</div><div class='section-title'>Order Queue</div>", unsafe_allow_html=True)
    orders = [
    ...
    for o in orders:
        ...
        with st.expander(f"{o['id']}  ·  {o['buyer']}  ·  {o['product']}  ·  ₹{o['value']:,}"):
            ...
            if o["status"]=="New":
                ...
                with b1:
                    if st.button("✅ Confirm", key=f"conf_{o['id']}"): st.success("Order confirmed!")
                with b2:
                    if st.button("❌ Decline", key=f"dec_{o['id']}"): st.error("Order declined.")
    st.markdown("</div>", unsafe_allow_html=True)

# ── FOOTER ────────────────────────────────────────────────────────────────────
st.markdown("""
<hr>
<div style='text-align:center;padding:20px;font-size:0.65rem;color:#888;letter-spacing:0.15em;text-transform:uppercase;'>
